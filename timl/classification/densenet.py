...
class DenseNet169Classifier(Classifier):
    ...
    def __init__(self,
        ...
        self._model = build_densenet169(n_classes=n_classses,
                                        dim=input_dim,
                                        fc_shape=fc_shape,
...
class DenseNet169ClassifierML(Classifier):
...
def build_densenet169(n_classes: int,
    ...
    """
    The factory method to setup a model based on DenseNet169.

    :param n_classes: The number of predictions for the final softmax/sigmoid
    ...
    :param weights: the initial weights of the model (defaults to imagenet)
    ...
    base_model = DenseNet169(weights=weights, input_shape=(dim[0], dim[1], 3), include_top=False)
    ...
    # Compose model
    model = models.Model(base_model.layers[0].output, x, name='DenseNet169')
