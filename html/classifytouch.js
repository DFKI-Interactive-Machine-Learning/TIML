...
function($scope, $http, $sce, $window, fileUpload)
{
    ...
    $scope.Filter_isPositive = function(prop){
        return function(item){
          return item[prop] > 0;
        }
    }
    function getImageStyle (probability_malign) {
        colors = ['green', 'yellow', 'darkorange', 'red'];
        bstyle = '2pt solid ';
        if      (probability_malign >= 0.75) bstyle += colors[3];
        else if (probability_malign >= 0.5)  bstyle += colors[2];
        else if (probability_malign >= 0.25) bstyle += colors[1];
        else                                 bstyle += colors[0];
	...
	function classifyImage (file) {
        ...
        $scope.classifiedImages[file.name].predict2 = [];
        $scope.classifiedImages[file.name].predict8 = [];
		...
		.success(function(response) {
			...
			if (response.prediction != undefined) {
				...
				for (i=0; i < response.prediction.length; i++){
				    predict2.push({
				    'class': $scope.modelInfo.binary_classification_classes[i],
				    'desc': $scope.modelInfo.binary_classification_classes_description[i],
				    'pred': response.prediction[i],
				    'conf': response.confidence[i]})
				    }
				$scope.classifiedImages[file.name].predict2 = predict2;
				$scope.StyleOrigImage = getImageStyle(predict2[1].pred)
				console.log("Orig Image Style: " , $scope.StyleOrigImage)
				console.log("binary", $scope.classifiedImages[file.name].predict2)
			...
			//classifyImage8(file, classifierUrl);
			//recClassify();
    ...
    function classifyImage8 (file) {
		var classifierUrl = $window.services.classifier8;
		var fd = new FormData();
		fd.append('file', file._file);
		...
		.success(function(response) {
			...
			if (response.prediction != undefined) {
				...
				for (i=0; i < response.prediction.length; i++){
				    predict8.push({
				    'class': $scope.modelInfo.eight_class_classification_classes[i],
				    'desc': $scope.modelInfo.eight_class_classification_classes_description[i],
				    'pred': response.prediction[i],
				    'conf': response.confidence[i]})
				    }
				$scope.classifiedImages[file.name].predict8 = predict8;
				console.log("8class", predict8)
    ...
    $scope.explainResults = function (file){
    	var fd = new FormData();
        fd.append('file', file._file);
        if (file.explanation) file.explanation.layers = [];
        file.explanation = {'layers': []};
        file.explainrunning = $scope.loadingGif;

        var visual = file.visual;
        var layers = file.layers, args = "";
        if (!file.risemasks) file.risemasks = 100;
        var noofmasks = Math.min(file.risemasks, 500)
        if (visual=='rise') {
            args = {'N': noofmasks};
            layers = "";
            cols = 1;
            console.log("visual==rise, args=" , args, JSON.stringify(args));
        }
        if (visual=='gradcam') {
            args = ""; // //args = {'layer_name': 'block5_pool'};// 'block5_conv3'
            cols = layers;
        }

        file.origlayer = {'id': "layerorig"+file.name, 'name': 'orig', 'src': file.url};
        var layerorig = document.getElementById("layerorig"+file.name);
        file.modalpanestyle = {'max-width': (cols * layerorig.width+ 100)+"px", 'height': (2*layerorig.height+150)+"px"};
        console.log("setting modal pane to ", file.modalpanestyle)
        //console.log("orig layer "+"layerorig"+file.imageID, layerorig)
        ...
        //console.log("visual " + visual , "call " , call)
    	...
    	.success(function(response){
    	    if (response.error) {
    	        file.explanation.error = response.error;
    	        console.log("layers error: ", response);
			    file.explainrunning = null;
			    return;
    	    }

            var explanation = {'layers': [], 'method': "", 'args': ""}
    	    var layerstyle = {width: layerorig.width+"px", height: layerorig.height+"px"};

    	    for (var i=0; i < response.layers.length; i++) {
    	        var curr = response.layers[i];
    	        explanation.layers.push({'name': curr['layer_name'], 'src': decodeImage(curr['composite']), 'style': layerstyle});
			}

            file.explanation = explanation;
			console.log("explanation: ", file.explanation);
			file.explainrunning = null;
    	})
    	.error(function(response){
    	    file.explanation.error = response;
    		console.log("layers error: ", response);
			file.explainrunning = null;
    	})
    }

    $scope.explainResultsDefaults = function (file, visual){
    	var fd = new FormData();
        fd.append('file', file._file);
        file.explanationdef = {};
        file.explanationdef[visual] = {'layers': []};

        file.explainrunning = $scope.loadingGif;

        var args = (visual == 'rise') ? {'N': 10} : {'layer_name': 'block5_conv3'};
        var call =
        	"/skincare/Explain"
        	+ "?visual=" + visual
        	+ "&layers=" + 2
    	...
    	.success(function(response){
    	    if (response.error) {
    	        file.explanationdef.error = response.error;
    	        console.log("layers error: ", response);
			    file.explainrunning = null;
			    return;
    	    }
    	    ...
    	    for (var i=0; i < response.layers.length; i++) {
    	        var curr = response.layers[i];
    	        explanation.layers.push({'name': curr['layer_name'], 'src': decodeImage(curr['composite'])});
			}
    	    //explanation.method = response.method;
    	    //explanation.args = response.args;
    	    //var layerorig = document.getElementById("layerorig"+file.imageID)
    	    //explanation.origlayer = {'name': 'orig', 'src': file.url};
    	    //layerstyle = {};
    	    //explanation.layers.push({'name': 'grey map', 'src': decodeImage(response.grey), 'style': layerstyle});
    	    //explanation.layers.push({'name': 'heat map', 'src': decodeImage(response.heatmap), 'style': layerstyle});
    	    //explanation.layers.push({'name': 'composite', 'src': decodeImage(response.comp), 'style': layerstyle});

            file.explanationdef[visual] = explanation;
    ...
    $scope.explainResultsXAI_OLD = function (file){
            var fd = new FormData();
            fd.append('file', file._file);
            file.explainrunning = $scope.loadingGif;
            var pseudoCall =
                "/IML/XAI2/Classifier?arch=googlenet&topk=1&cuda=true&target_layers=inception5b,inception4d,inception4b,inception3b&visual=gradcam";
            $http.post(pseudoCall, fd,
                        {transformRequest: angular.identity,
                            headers: {'Content-Type': undefined}
                     })
            .success(function(response){
                var response_predictions = response.Predictions;
                var layers = [];
                for (var i=0; i<response_predictions.length;i++) {
                    var pred = response_predictions[i];
                    pred.layers.forEach(function(layer) {
                        layers.push({"src": decodeImage(layer.image), "name": layer.layer})
                    })
